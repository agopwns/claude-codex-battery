...
- 상태(플러그인이 관리): `.usage-snapshot.json`(수집 캐시 — 렌더는 이것만 읽고, `--collect` 분리 프로세스가 갱신) `.collect.lock`(수집 중복 방지, TTL 90s) `.claude-usage.json`(usage API 캐시) `usage-history.json`(일별 스냅샷, ROI 데이터 — 삭제 금지) `.batt-burn.json`(C5 샘플 + `pace` 페이스 코치 결과) `.batt-notify-state.json` `.batt-budget-state.json` `.batt-landing-state.json`(월말 예측 알림 래칫) `.batt-anomaly-state.json`(지출 이상 하루 1회) `.batt-reset-reminder-state.json`(리셋 임박 알림 래칫) `.pet-events.json`(본체→펫 이벤트 링) `.pet-event-seen.json`(펫 이벤트 중복 방지) `.pet-bubble-msg.json` `.pet-bubble-shown.json` `.update-check.json`
