...
// ── 펫 이벤트 버스: 순간 이벤트를 링(최대 10개)으로 기록 — 펫이 읽어 이벤트 애니메이션 재생 ──
// 알림(NOTIFY)과 독립 — 알림을 꺼도 펫은 반응한다. priority 낮을수록 우선(1=최우선).
const PET_EVENTS_FILE = `${HOME}/.claude/swiftbar/.pet-events.json`;
function emitPetEvent(type, ttlSec = 600, priority = 5) {
  try {
    let ring = [];
    try {
      const p = JSON.parse(readFileSync(PET_EVENTS_FILE, "utf8"));
      if (Array.isArray(p)) ring = p;
    } catch {}
    ring.push({
      id: `${type}-${now}`,
      type,
    ...
    ring = ring.filter((e) => e && e.expiresAt > now).slice(-10);
    mkdirSync(dirname(PET_EVENTS_FILE), { recursive: true });
    writeFileSync(PET_EVENTS_FILE, JSON.stringify(ring));
  } catch {}
}
...
function checkBudgetAlert(curSum) {
  try {
    ...
    if (order[zone] > order[state.zone]) {
      if (zone === "over100") emitPetEvent("budget-over", 600, 2); // 펫: 예산 초과 반응
      if (NOTIFY !== "off") {
        if (zone === "warn80") {
          fireNotification(
...
function runawayRow(hist, cmodels) {
  try {
    ...
    try {
      ...
      if (!st.notified) {
        emitPetEvent("spend-anomaly", 600, 1); // 펫: 지출 폭주 반응
        if (NOTIFY !== "off")
          fireNotification(
...
let worstZone = "ok"; // 말풍선용: 이번 실행에서 관측된 가장 나쁜 배터리 존
{
  ...
  for (const item of battItems) {
    ...
    const resetRecovered =
      (prevZone === "low20" || prevZone === "low10") &&
      zone === "ok" &&
      remain >= 95;
    if (resetRecovered) emitPetEvent("quota-reset", 600, 3); // 펫: 리셋 축하 (알림과 독립)
